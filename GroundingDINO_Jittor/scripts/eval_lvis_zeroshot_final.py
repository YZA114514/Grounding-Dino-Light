...
# -*- coding: utf-8 -*-
"""
Grounding DINO Jittor - LVIS Zero-Shot 评估脚本 (FINAL VERSION)

修复了以下关键问题：
1. 后处理逻辑错误 - 正确计算每个query的置信度
2. 数值稳定性 - 处理极端logit值
3. 真正的零样本评估 - 使用所有类别而非GT类别
4. 正确的token到类别映射 - 处理BERT多token映射
5. Jittor argmax返回tuple问题

用法:
    python scripts/eval_lvis_zeroshot_final.py \
        --checkpoint weights/groundingdino_swint_ogc_jittor.pkl \
        --lvis_ann data/lvis/lvis_v1_val.json \
        --image_dir data/coco/val2017 \
        --output_dir outputs/lvis_zeroshot_final
"""
...
class Config:
    ...
    def __init__(self):
        ...
        self.prompt_batch_size = 25     # 每个prompt包含的类别数
...
class TokenToCategoryMapper:
    """Token位置到类别的映射器"""
    
    def __init__(self, model, category_names: List[str], prompt_batch_size: int = 25):
        """
        初始化映射器
        ...
        Args:
            ...
            prompt_batch_size: 每个prompt包含的类别数
        """
        self.model = model
    ...
    def _build_mapping(self):
        """构建token位置到类别的映射"""
        print("Building token-to-category mapping...")
        
        # 分批处理类别
        category_index = 0
        for batch_idx in range(0, len(self.category_names), self.prompt_batch_size):
            batch_names = self.category_names[batch_idx:batch_idx + self.prompt_batch_size]
            prompt = '. '.join(batch_names) + '.'
            
            print(f"Processing batch {batch_idx//self.prompt_batch_size + 1}: {prompt[:100]}...")
            
            # 编码文本
            with jt.no_grad():
                text_features = self.model.text_encoder(prompt)
            
            # 获取token mask
            mask = text_features['text_token_mask'].numpy()
            if mask.ndim > 1:
                mask = mask[0]
            
            # 找到有效的token位置
            valid_positions = [i for i, m in enumerate(mask) if m]
            
            # 简化映射：假设每个类别占用连续的token位置
            # 这是近似方法，因为BERT的tokenization更复杂
            tokens_per_category = len(valid_positions) // len(batch_names)
            
            for cat_idx, cat_name in enumerate(batch_names):
                start_pos = cat_idx * tokens_per_category
                end_pos = min((cat_idx + 1) * tokens_per_category, len(valid_positions))
                
                # 为这个类别的所有token位置映射到同一个类别索引
                for token_pos in valid_positions[start_pos:end_pos]:
                    self.token_to_category[token_pos] = category_index
                
                print(f"  {cat_name} (cat_idx={category_index}) -> tokens {valid_positions[start_pos:end_pos]}")
                category_index += 1
            
            print(f"  Valid tokens: {valid_positions}")
            print(f"  Tokens per category: {tokens_per_category}")
        
        print(f"Built mapping for {len(self.token_to_category)} token positions")
    
    def token_to_category_index(self, token_pos: int) -> int:
        """
        将token位置转换为类别索引
        ...
        Returns:
            类别索引，如果未找到返回-1
        """
        # 处理浮点数token位置（来自argmax的bug）
        if isinstance(token_pos, float):
            token_pos = int(round(token_pos))
        
        return self.token_to_category.get(token_pos, -1)
...
class LVISEvaluationDataset:
    ...
    def get_zero_shot_prompts(self, batch_size: int = 25) -> List[str]:
        """
...
def postprocess_predictions(
    ...
    config: Config,
    token_mapper: TokenToCategoryMapper
) -> List[Dict]:
    """
    后处理模型输出 (最终版本)
    ...
    max_probs, pred_token_positions = jt.max(pred_probs, dim=-1)  # Jittor max returns (values, indices)
    
    # FIX 3: 正确处理维度
    if max_probs.ndim > 1:
        max_probs = max_probs.squeeze(-1)
    if pred_token_positions.ndim > 1:
        pred_token_positions = pred_token_positions.squeeze(-1)
    
    print(f"DEBUG: max_probs shape: {max_probs.shape}")
    print(f"DEBUG: pred_token_positions shape: {pred_token_positions.shape}")
    print(f"DEBUG: max_probs range: [{max_probs.min().item():.4f}, {max_probs.max().item():.4f}]")
    print(f"DEBUG: pred_token_positions range: [{pred_token_positions.min().item()}, {pred_token_positions.max().item()}]")
    ...
    token_positions = pred_token_positions[mask].numpy()
    boxes = pred_boxes[mask].numpy()
    
    print(f"DEBUG: Valid predictions - scores range: [{scores.min():.4f}, {scores.max():.4f}]")
    print(f"DEBUG: Valid predictions - token_positions range: [{token_positions.min()}, {token_positions.max()}]")
    
    # FIX 5: 将token位置映射到类别索引
    category_indices = []
    for token_pos in token_positions:
        cat_idx = token_mapper.token_to_category_index(token_pos)
    ...
    for i in range(len(scores)):
        ...
        if w > 0 and h > 0:
            results.append({
                ...
                'token_pos': int(token_positions[i])  # 保存token位置用于调试
            })
...
def visualize_prediction(
    ...
    for res in results:
        ...
        token_pos = res.get('token_pos', -1)
        
        if 0 <= cat_idx < len(category_names):
            label = f"{category_names[cat_idx]} (cat={cat_idx}, tok={token_pos}): {res['score']:.2f}"
        else:
            label = f"Cat_{cat_idx} (tok={token_pos}): {res['score']:.2f}"
...
def evaluate_lvis(
    ...
    # 创建token到类别映射器
    token_mapper = TokenToCategoryMapper(model, dataset.category_names, config.prompt_batch_size)
    ...
    for idx in tqdm(range(num_images), desc="Evaluating"):
        ...
        # 使用第一个prompt进行评估（包含前25个类别）
        caption = zero_shot_prompts[0]
        ...
        # 后处理
        results = postprocess_predictions(outputs, orig_size, config, token_mapper)
        ...
        for i, res in enumerate(results[:10]):  # 显示前10个
            cat_idx = res['category_idx']
            token_pos = res.get('token_pos', -1)
            cat_name = f"Unknown_{cat_idx}"
            if 0 <= cat_idx < len(dataset.category_names):
                cat_name = dataset.category_names[cat_idx]
            print(f"  {i+1}: {cat_name} (cat_idx={cat_idx}, tok={token_pos}) score={res['score']:.3f}")
        
        # 统计类别分布
        if results:
            cat_counts = defaultdict(int)
            for res in results:
                cat_counts[res['category_idx']] += 1
            print(f"Category distribution: {dict(list(cat_counts.items())[:5])}")  # 显示前5个类别
        ...
        for r in results:
            ...
            # 确保类别索引有效
            if 0 <= cat_idx < len(dataset.cat_ids):
...
def main():
    parser = argparse.ArgumentParser(description='LVIS Zero-Shot Evaluation (Final Version)')
    ...
    parser.add_argument('--output_dir', type=str, default='outputs/lvis_zeroshot_final',
    ...
    print("=" * 60)
    print("LVIS Zero-Shot Evaluation (Final Version)")
    print("=" * 60)
    ...
    # 打印结果
    print("\n" + "=" * 60)
    print("Evaluation Results (Final Version)")
    print("=" * 60)
