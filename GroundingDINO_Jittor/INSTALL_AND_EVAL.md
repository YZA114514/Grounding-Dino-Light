...
1. **下载 LVIS 标注文件**：
   - `lvis_v1_val.json` → 放置到 `data/lvis/`

2. **下载 COCO val2017 图像**：
   - 图像文件 → 放置到 `data/coco/val2017/`
...
```bash
nohup python scripts/eval_lvis_zeroshot.py \
    --checkpoint weights/groundingdino_swint_ogc_jittor.pkl \
    --lvis_ann data/lvis/lvis_v1_val.json \
    --image_dir data/coco/val2017 \
    --output_dir outputs/lvis_zeroshot \
    --use_gpu > lvis_zeroshot_run.log 2>&1 &
```
